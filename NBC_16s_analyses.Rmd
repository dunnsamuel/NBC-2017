...
env.df.fac=env.df[,1:2] #We want to copy the first two columns of site  data here.  We can't have ANY site data present when we do our ordination.  We will reattache this site data tot he ordination table afterwards so we can make inferences and good figures.
...
bac.df.sps<-bac.env %>% 
  spread(key=taxon, value=obs) %>% #spread take a tall data table and makes it wide.  each "key" value becomes a column populated by the "value"
...
## Ellipses.  The following tow functions are custom creations to generate elipses for our NMDS plot
...
bac.df_ell <- data.frame()
  for(g in levels(bac.ORD$Substrate)){ # you will have to update some of these fields ....look for bac.ORD, Substrate, etc.
...
bac.nmds=ggplot(bac.ORD)+
  geom_point(mapping= aes(MDS1,MDS2,shape=Date_Sampled,color=Substrate))+
  coord_fixed(ratio=1)+
  geom_segment(data=bac.spp.scrs, #lines for vectors
               aes(x=0, xend=bac.spp.scrs$NMDS1/4, y=0,yend=bac.spp.scrs$NMDS2/4),
               arrow=arrow(length=unit(.1, "cm")),
               color="red")+
  geom_text(data = bac.spp.scrs, #labels for vectors
            aes(x = NMDS1/4.5, y = NMDS2/4.5, label = Assay),
            size = 3) +
  geom_path(data=bac.df_ell, #ellipses
              aes(x=MDS1, y=MDS2,colour=group), 
              size=1, linetype=2)+
  ggtitle("Bacterial Communities")+
  theme_classic()+
  labs(shape="Date Sampled", 
       color="Substrate")+
  xlim(-0.3,0.3)+
  ylim(-0.3,0.3)
bac.nmds
...
bac.perm.tab<-rownames_to_column(bac.perm.tab,var="Term") #createing a term column from rownames
