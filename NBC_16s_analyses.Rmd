...
bac.rab<-bac.rab %>%   #rabund calculations
  group_by(Sample) %>% 
...
rabund.plots<-rabund.sum.df %>% #generates list of plots by Date (groubp_by command)  
  ...
  do(plots=ggplot(.,aes(x=Substrate,y=mean_rabund,group=taxon,color=taxon,fill=taxon))+
       ...
       xlab("Substrate")+
       theme_classic())
rabund.plots$plots[[1]] #calls first plot from list of plots
...
require(viridis)
ggplot(rabund.sum.df, aes(Date,taxon,fill=mean_rabund))+geom_tile()+
  facet_wrap(~Substrate,nrow=1)+scale_fill_viridis()+
