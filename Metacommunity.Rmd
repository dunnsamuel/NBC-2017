...
packages <- c("ggplot2", 
              ...
              "gapminder","magrittr","dplyr","ggpubr","gridExtra","MASS","patternplot","tibble","gplots","broom","data.table","devtools","lubridate","caret","glmulti")
ipak(packages)
...
pos.dat<- dplyr::select(pos.dat,-c(Rep,obs,doy,Date,total_obs,Substrate,nep_dif))
neg.dat<- dplyr::select(neg.dat,-c(Rep,obs,doy,Date,total_obs,Substrate,nep_dif))
mod.dat<- dplyr::select(mod.dat,-c(Rep,obs,doy,Date,total_obs,Substrate,nep_dif))
...
mod_pos.final<-lm(NEP ~ 1 + GPP + Resp + time + srp + no3 + nh4 + Flavobacteria + Verrucomicrobiae, data=pos.dat)
summary(mod_pos.final)


mod_neg.final<-lm(NEP ~ 1 + GPP + Resp + time + c__Leotiomycetes + k__Fungi_unclassified,data=neg.dat)
summary(mod_neg.final)


full_mod.final<-lm(NEP ~ 1 + GPP + Resp + Actinobacteria + Verrucomicrobiae,data=mod.dat)
summary(full_mod.final)
