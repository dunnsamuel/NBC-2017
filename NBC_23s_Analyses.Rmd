...
algae.df=bacteria.df[algae.df$taxon !="Chloroplast_ge",]
...
shannon.nbc=apply(algae.df.6[,6:67],2,shannon)
...
sites=list(colnames(algae.df.5[,6:67])) #get list of samples
...
rabund.23df=algae.df.5[,6:67]#rabund(algae.df.4[,6:71])
...
rabund.23h=aggregate(rabund.23df[,4:79],list(rabund.23df$Date,rabund.23df$Substrate),mean)
...
anosim.df=anosim.df%>%
  rownames_to_column
anosim.df[,2:682]=lapply(anosim.df[,2:682],function(x) as.numeric(as.character(x)))
