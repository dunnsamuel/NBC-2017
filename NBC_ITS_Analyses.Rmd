...
its.df.3=subset.data.frame(its.df, taxlevel==4,drop=TRUE) %>% 
...
its.df.sp$Substrate=factor(its.df.sp$Substrate,levels=c("H","So","Sh","F","T"),
                           labels=c("PVC","Soft PE","Sheet PE","PS","Tile"))


end.df.names=c("Date","Substrate")
...
its.nmds=ggplot(its.ORD)+
  geom_point(mapping= aes(MDS1,MDS2,shape=Date_Sampled,color=Substrate,size=Date_Sampled))+
  coord_fixed(ratio=1)+
