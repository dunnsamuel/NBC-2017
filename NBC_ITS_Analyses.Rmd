...
its.df<-read.delim(file="nbcits.final.an.0.01.cons.tax.summary.txt")
...
shannon.nbc=apply(its.df.6[,6:72],2,shannon)
...
tidy(TukeyHSD(aov(value~Date,data=shannon.df)))
...
its.shan.tukey.out=shannon.df%>%
  group_by(Date)%>%
  do(tidy(TukeyHSD(aov(value~Substrate,data=.))))
its.shan.tukey.out
...
otu=apply(its.df.6[,6:65]>=observationThreshold, 2, sum,na.rm=T) #check by row to see if each taxa present or not
...
sites=list(colnames(its.df.5[,6:74])) #get list of samples
